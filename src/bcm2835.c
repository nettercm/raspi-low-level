...
volatile uint32_t *bcm2835_irq	= MAP_FAILED;
volatile uint32_t *bcm2835_uart0= MAP_FAILED;
...
void bcm2835_delayMicroseconds(uint64_t micros)
{
    ...
    {
	t1.tv_sec = 0;
	t1.tv_nsec = 1000 * (long)(micros - 50); //-200
	nanosleep(&t1, NULL);
    }    
...
int bcm2835_init(void)
{
    ...
    // IRQ
    bcm2835_irq = mapmem("irq", BCM2835_BLOCK_SIZE, memfd, BCM2835_IRQ_BASE);
    if (bcm2835_irq == MAP_FAILED) goto exit;

    // UART0
    bcm2835_uart0 = mapmem("irq", BCM2835_BLOCK_SIZE, memfd, BCM2835_UART0_BASE);
    if (bcm2835_uart0 == MAP_FAILED) goto exit;
